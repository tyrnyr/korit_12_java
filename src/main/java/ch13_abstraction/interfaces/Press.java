...
public interface Press {
    ...
    /*
        ...
        그래서 String 앞에 final을 적더라도 Interllij상에서 불필요하다고
        안내가 나옵니다.
...
//    }; // 인터페이스 상에서는 추상 메서드만 가능합니다. 추상 메서드가 뭐다? 메서드 본문이 없는 메서드
         // 나머지 로직은 서브 클래스에서 '구현'해야 했씁니다.

    // 되는 예시
    void onPressed();   // 이거 call1() ~ call4() 중에 무슨 유형이고 그 근거는 무엇일까요? -> call1()

    /*
        Up interface를 생성하고 onUp()이라는 추상 메서드를 정의 하시오 - > call1()
        Down interface를 생성하고 onDown()이라는 추상 메서드를 정의 하시오 -> call1()
     */
}
